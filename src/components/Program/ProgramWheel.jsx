...
export default function ProgramWheel() {
  ...
  // Utiliser les données du CMS ou retourner vide
  const PROGRAM = (programme && programme.length > 0) 
    ? programme.map((p, idx) => ({
        ...
        icon: p.icon || '✦',
        color: '#C9A84C',
        image: p.image_url || 'https://images.unsplash.com/photo-1518998053901-5348d3961a04?w=600&q=80',
      }))
    : []
              ...
              {PROGRAM.map((step, i) => {
                ...
                return (
                  <motion.path
                    ...
                    fill={isActive ? 'rgba(201,168,76,0.15)' : 'transparent'}
